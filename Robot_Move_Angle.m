function Robot_Move_Angle(phi_in,theta_in,Tool,speed)
...
if pitch_read == 9999 && yaw_read == 9999
    printf('The Error occur when read angle! Please check the error manually.');
    return;
else
    ...
    if abs(diff_phi) > 100
        ...
        while (abs(yaw_read) >= 100)
            ...
            if pitch_read == 9999 && yaw_read == 9999 || loopcnt  > 100             %Loop count more than 100
                ...
                diff_phi = phi_in - pitch_read;                         %pitch    
                diff_theta = theta_in - yaw_read;                       %yaw
                Tx     = typecast(int32(-yaw_read*100),'uint8');            %yaw     
                ...
                pause(abs(yaw_read/100)/speed+0.02);
            end
        end
        %change pitch
        loopcnt = 0;
        ...
        Tx     = typecast(int32(diff_theta),'uint8');       %yaw
        Ty     = typecast(int32(0),'uint8');                %pitch
        Axis    = [X,Y,Z,Tx,Ty,Tz];
        Command = [Head,Suh,Setting,Axis, Reserv, Type, Ex, ToolNo, Coor, Ext];
        robot=UDPNode('192.168.2.250',10040, 500);
        ...
        while(abs(diff_theta) > 100)
            ...
            if pitch_read == 9999 && yaw_read == 9999 || loopcnt  > 100             %Loop count more than 100
                ...
                robot=UDPNode('192.168.2.250',10040, 500);
                robot.submit(Command);
                pause(abs(diff_theta/100)/speed+0.05);
            end
